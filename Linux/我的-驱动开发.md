...
1. **选择板级配置**
	...
	./build.sh lunch
	```
	出现的可选项对应`<SDK>/device/rockchip/rk356x/`目录下的配置文件
2. **编译所有**
	``` bash
	./build.sh all
	```
3. **将编译的镜像打包到`<SDK>/rockdev/`目录下**
	``` bash
	./build.sh firmware
	```
4. **将`<SDK>/rockdev/`目录下的镜像文件打包成`update.img`**
	``` bash
	./build.sh updateimg
	```
- **单独编译 kernel**
...
./make.sh rk3568
```
	编译后生成`uboot.img`与`rk356x_spl_loader_v1.13.112.bin`
### 3.2 U-Boot 的设备树
`<U-Boot>/arch/arm/dts/rk3568-evb.dts`
### 3.3 关键镜像
1. `uboot.img`
	- 由多个镜像合并而成，是一种 FIT（flattened image tree）格式镜像， 支持任意多个 image 打包和校验
	- FIT 镜像使用 its 文件来描述 image 的信息，最终通过`<U-Boot>/tools/mkimage`工具生成 itb 镜像，its 文件和生成的 itb 文件都在 `<U-Boot>/fit`目录下
	- u-boot.its 文件中描述了有哪些镜像会参与合并成`uboot.itb`有如下镜像
		`u-boot-nodtb.bin`
		`u-boot.dtb`
		`bl31_0x00040000.bin`
		`bl31_0x00068000.bin`
		...
		`bl31_0xfdcd0000.bin`
		`tee.bin`
		- ARM Trusted Firmware 固件（ATF）在编译时由 `<SDK>/rkbin/bin/rk35/rk3568_bl31_v1.33.elf`拷贝到`<U-Boot>/bl31.elf`，拷贝后的文件并不直接打包进`uboot.itb`，而是分解成多个`bl31_xxx.bin`文件，最终将这些`bl31_xxx.bin`镜像打包进`uboot.itb`
		- OP-TEE 固件在编译时`<SDK>/rkbin/bin/rk35/rk3568_bl32_v2.08.bin`拷贝到`<U-Boot>/tee.bin`
	- ==uboot.img 镜像就是多份 itb 镜像==
		`uboot.img = uboot.itb * N(N一般是2)`
2. `rk356x_spl_loader_v1.13.112.bin`
	...
	- 默认使用闭源的TPL与SPL
		`<SDK>/rkbin/bin/rk35/rk3568_ddr_1560MHz_v1.13.bin`称为 ddr bin
		`<SDK>/rkbin/bin/rk35/rk356x_spl_v1.12.bin`称为 miniloader
### 3.4 镜像启动顺序
Maskrom->TPL->SPL->Trust(ATF + OP-TEE)->u-boot->kernel->rootfs
### 3.5 修改 U-Boot 配置
``` bash
<SDK>/u-boot
...
### 4.2 关键镜像
1. `boot.img`镜像
	- RK3568 平台 Linux 系统使用的 `boot.img` 是一种 FIT 格式镜像，它由多个镜像合并而成， 对于 RK3568 平台来说，烧录到开发板 boot 分区的 boot.img 包含了内核镜像 Image、内核 DTB、 resource.img 这三部分
	- 使用`./make,sh`编译生成的镜像不是 FIT 格式的
	- 使用如下命令将分散的镜像（内核镜像、内核DTB、资源镜像）打包成 FIT 格式的`boot.img`镜像
		``` bash
...
- TX_CLK：发送时钟，如果网速为 100M 的话时钟频率为 25MHz，10M 网速的话时钟频率 为 2.5MHz，==此时钟由 PHY 产生==并发送给 MAC。
- TX_EN：发送使能信号。
...
- REF_CLK：参考时钟，==由外部时钟源（晶振）提供==，频率为 50MHz。这里与 MII 不同，MII 的接 收和发送时钟是独立分开的，而且都是由 PHY 芯片提供的。
## GMII 接口
...
- GTX_CLK：1000M 工作模式下的发送时钟（125MHz）。MAX->PHY
- TX_EN：发送使能信号。
...
## 地址
### 1. 物理地址
每块网卡理论上都有一个全球唯一的标识ID，把这个ID号叫做物理地址，也叫做MAC地址，MAC地址由48位组成
### 2. IP 地址
标识主机或网络的虚拟地址（可以修改），IP地址有IPV4与IPV6
### 3. PHY芯片地址
通过PHY地址访问指定的PHY芯片
## Linux 内核网络驱动框架
### net_device 结构体

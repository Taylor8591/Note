...
static uint32_t bl0939_read_reg(int fd, uint8_t reg) {
    ...
    printf("tx data: %02x %02x %02x %02x %02x %02x\n", tx[0], tx[1], tx[2], tx[3], tx[4], tx[5]);
    printf("rx data: %02x %02x %02x %02x %02x %02x\n", rx[0], rx[1], rx[2], rx[3], rx[4], rx[5]);
...
static int bl0939_write_reg(int fd, uint8_t reg, uint32_t val, int check) {
    ...
    printf("read back reg 0x%02x: %x    ", reg, r_temp);
    printf("val: %x    ", val);
    if (r_temp == val) {
        printf("success\n");
        return 0;
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        // while(1) {
        //     // if(bl0939_write_reg(fd, 0x19, 0x005a5a5a, 1))  //复位用户寄存器(0x19, 0x005a5a5a, 1)
        //     // {
        //     //     printf("bl0939_write_reg 0x19 error\r\n");
        //     //     ret = -1;
        //     //     break;
        //     // }
        //     // else {
        //     //     printf("bl0939_write_reg 0x19 success\r\n");
        //     // }
        //     bl0939_reset(fd);
        //     while(1);
        // }

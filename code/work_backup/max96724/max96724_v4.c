...
static int max96712_get_mbus_config(struct v4l2_subdev *sd, unsigned int pad,
				    struct v4l2_mbus_config *cfg)
{
	struct max96712_priv *priv = v4l2_get_subdevdata(sd);

	cfg->type = priv->cphy ? V4L2_MBUS_CSI2_CPHY : V4L2_MBUS_CSI2_DPHY;
			    ...
			    struct v4l2_subdev_format *format)
{
	...
	/* Only support 1920x1080 in this colorbar test driver */
...
/* max96712_init_state function removed for kernel 6.1 compatibility */

/* internal_ops removed for kernel 6.1 compatibility */

static const struct v4l2_subdev_pad_ops max96712_pad_ops = {
	.get_fmt = v4l2_subdev_get_fmt,
	.set_fmt = max96712_set_fmt,
	.get_mbus_config = max96712_get_mbus_config,
};
...
static int max96712_v4l2_register(struct max96712_priv *priv)
{
	...
	/* Manually initialize default format since init_state is not available in kernel 6.1 */
	{
		struct v4l2_subdev_state *state;
		struct v4l2_mbus_framefmt *fmt;
		...
		if (state) {
			fmt = v4l2_subdev_get_try_format(&priv->sd, state, 0);
			if (fmt)
				*fmt = default_fmt;
...
static int max96712_parse_dt(struct max96712_priv *priv)
{
	...
	case V4L2_MBUS_CSI2_DPHY:
		supported_lanes = 4;
		priv->cphy = false;
		dev_info(&priv->client->dev, "Using D-PHY\n");
		break;
	case V4L2_MBUS_CSI2_CPHY:
		supported_lanes = 3;
		priv->cphy = true;
		dev_info(&priv->client->dev, "Using C-PHY\n");
		break;
	default:
	...
	if (v4l2_ep.bus.mipi_csi2.num_data_lanes != supported_lanes) {
		...
		dev_info(&priv->client->dev, "Number of data lanes: %u\n",
...
static int max96712_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	priv->info = device_get_match_data(&client->dev);
	...
	if (IS_ERR(priv->gpiod_pwdn))
		...
		dev_info(&client->dev, "PWDN GPIO acquired\n");
...
static const struct of_device_id max96712_of_table[] = {
	{ .compatible = "max96712", .data = &max96712_info_max96712 },
	{ .compatible = "max96724", .data = &max96712_info_max96724 },
	{ /* sentinel */ }
};

...
static const struct regmap_config ov8858_regmap_config = {
	.reg_bits = 16,
	.val_bits = 8,
	.max_register = 0x1272,
};
...
static const struct regval ov8858_3264x2448_regs_r2a_4lane[] = {
	...
	/* -------- Video Timing (3264x2448@30 RAW10) -------- */
	{ 0x1055, 0x00 }, /* VS_HIGH_2 */
	{ 0x1056, 0x36 }, /* VS_HIGH_1 */
	{ 0x1057, 0x00 }, /* VS_HIGH_0 */

	{ 0x1058, 0x0C }, /* VS_LOW_2 */
	{ 0x1059, 0xE9 }, /* VS_LOW_1 */
	{ 0x105A, 0x18 }, /* VS_LOW_0 */
	...
	{ 0x1067, 0x0C }, /* DE_HIGH_1 */
	{ 0x1068, 0xC0 }, /* DE_HIGH_0 */

	{ 0x1069, 0x00 }, /* DE_LOW_1 */
	{ 0x106A, 0xA8 }, /* DE_LOW_0 */

	{ 0x106B, 0x09 }, /* DE_CNT_1 */
	{ 0x106C, 0x90 }, /* DE_CNT_0 */

	/* -------- Pattern Generator Colors -------- */
	{ 0x1070, 0xFF }, /* CHKR_COLOR_A_H = R */
	{ 0x106F, 0x00 }, /* CHKR_COLOR_A_M = G */
	{ 0x106E, 0x00 }, /* CHKR_COLOR_A_L = B */

	{ 0x1073, 0x00 }, /* CHKR_COLOR_B_H = R */
	{ 0x1072, 0xFF }, /* CHKR_COLOR_B_M = G */
	{ 0x1071, 0x00 }, /* CHKR_COLOR_B_L = B */

	/* Checker pattern width / repeat */
	{ 0x1074, 0x80 }, /* CHKR_RPT_A */
	{ 0x1075, 0x80 }, /* CHKR_RPT_B */
	{ 0x1076, 0x01 }, /* CHKR_ALT */

	/* -------- Pattern Generator Control -------- */
	{ 0x1051, 0x10 }, /* PATGEN_MODE=01 (Checkerboard) */
	{ 0x1050, 0xE2 }, /* GEN_VS/HS/DE=1, VTG_MODE=10 (Auto-repeat) */

	/* -------- MIPI TX Output Format (RAW10) -------- */
	{ 0x00F0, 0x60 }, // 关联图案生成器与 Pipe 0
	{ 0x00F4, 0x11 },
	
	{ 0x08C0, 0x03 }, // 启用图案生成器功能
...
static long ov8858_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_QUICK_STREAM:
		...
		dev_info(&ov8858->client->dev, "ov8858_ioctl():write\n");
		if (stream)
...
{
	int ret;
	dev_info(&ov8858->client->dev, "__ov8858_start_stream():start write registers\n");
	ret = ov8858_write_array(ov8858->client, ov8858->cur_mode->reg_list);
	if (ret)
		return ret;
	dev_info(&ov8858->client->dev, "__ov8858_start_stream():finish write registers, success\n");
	...
	if (ret)
		return ret;
	dev_info(&ov8858->client->dev, "__ov8858_start_stream():finish ctrl setup, success\n");
	return ov8858_write_reg(ov8858->client,
				OV8858_REG_CTRL_MODE,
				OV8858_REG_VALUE_08BIT,
				OV8858_MODE_STREAMING);
...
{
	dev_info(&ov8858->client->dev, "__ov8858_stop_stream():finish\n");
	return ov8858_write_reg(ov8858->client,
				OV8858_REG_CTRL_MODE,
				OV8858_REG_VALUE_08BIT,
				OV8858_MODE_SW_STANDBY);
...
static int ov8858_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		// 	dev_info(&client->dev, "=== MAX96724 Status Check ===\n");
			
		// 	/* 读取VIDEO_LOCK状态 (0x1DC) */
		// 	regmap_read(ov8858->regmap, 0x1DC, &val);
		// 	dev_info(&client->dev, "VIDEO_LOCK (0x1DC): 0x%02x (bit0=1:locked)\n", val);
			
		// 	/* 读取链路锁定状态 (0x001A) */
		// 	regmap_read(ov8858->regmap, 0x001A, &val);
		// 	dev_info(&client->dev, "Link Lock Status (0x001A): 0x%02x (bit0=1:locked)\n", val);
			
		// 	/* 读取视频锁定状态 (0x0107) */
		// 	regmap_read(ov8858->regmap, 0x0107, &val);
		// 	dev_info(&client->dev, "Video Lock Status (0x0107): 0x%02x (bit7=1:locked)\n", val);
			
		// 	/* 读取CSI输出使能状态 (0x0332) */
		// 	regmap_read(ov8858->regmap, 0x0332, &val);
		// 	dev_info(&client->dev, "CSI Output Enable (0x0332): 0x%02x (should be 0xE4)\n", val);
			
		// 	/* 读取错误状态 (0x0013) */
		// 	regmap_read(ov8858->regmap, 0x0013, &val);
		// 	dev_info(&client->dev, "Link Error Status (0x0013): 0x%02x\n", val);
			
		// 	/* 读取视频管道错误 (0x0108) */
		// 	regmap_read(ov8858->regmap, 0x0108, &val);
		// 	dev_info(&client->dev, "Video Pipe Error (0x0108): 0x%02x\n", val);
			
		// 	dev_info(&client->dev, "=== End of Status Check ===\n");
		else {
...
static int ov8858_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		dev_info(&client->dev, "ov8858_s_power():start write registers\n");
		...
		dev_info(&client->dev, "ov8858_s_power():finish write registers, success\n");
...
/*
	...
	v4l2-ctl -d /dev/video0 --stream-mmap --stream-count=10
		ov8858_s_stream() (开始流)
		结束时再次调用 ov8858_s_stream() (停止流)

	v4l2-ctl -d /dev/video0 --list-ctrls
		ov8858_core_ops 中的控制相关函数

	cat /dev/video0
		ov8858_s_power()

	media-ctl -d /dev/media0 --set-v4l2 '"ov8858":0[fmt:SBGGR10_1X10/3264x2448]'
		ov8858_set_fmt
*/
				   ...
				   struct i2c_client *client)
{
	...
	if (id == CHIP_ID) {
		if (4 == ov8858->lane_num) {
			ov8858_global_regs = ov8858_global_regs_r2a_4lane; // ov8858_s_power
			ov8858->cur_mode = &supported_modes_r2a_4lane[0]; // __ov8858_start_stream
			 ...
			 const struct i2c_device_id *id)
{
	...
	/* Initialize regmap */
	ov8858->regmap = devm_regmap_init_i2c(client, &ov8858_regmap_config);
	if (IS_ERR(ov8858->regmap)) {
		dev_err(dev, "Failed to initialize regmap\n");
		return PTR_ERR(ov8858->regmap);
	}
	...
	dev_info(dev, "ov8858_check_sensor_id success\n");	
	...
	dev_info(dev, "OV8858 probed successfully\n");

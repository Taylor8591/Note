			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	mutex_lock(&ov8858->mutex);
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {		// TRY 格式（测试格式）
#ifdef CONFIG_VIDEO_V4L2_SUBDEV_API
...
static int ov8858_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	switch (ctrl->id) {
	case V4L2_CID_VBLANK:	// 当设置垂直消隐（VBLANK）时，动态调整曝光时间的最大值
	...
	switch (ctrl->id) {
	case V4L2_CID_EXPOSURE:			// 设置曝光时间（单位：行时间）
		...
		ret = ov8858_write_reg(ov8858->client,
					...
					ctrl->val << 4);
		break;
	case V4L2_CID_ANALOGUE_GAIN:	// 设置模拟增益（控制传感器灵敏度）
		...
		break;
	case V4L2_CID_VBLANK:			// 设置 VTS (Vertical Total Size) = 垂直总行数
		...
		break;
	case V4L2_CID_TEST_PATTERN:		// 启用/禁用测试图案（用于调试）
...
static int ov8858_parse_of(struct ov8858 *ov8858)
{
	...
	if (4 == ov8858->lane_num) {
		ov8858->cur_mode = &supported_modes_r2a_4lane[0];
		supported_modes = supported_modes_r2a_4lane;
		ov8858->cfg_num = ARRAY_SIZE(supported_modes_r2a_4lane);	// 存储该传感器支持的配置模式总数
			 ...
			 const struct i2c_device_id *id)
{
	...
	sd->flags |= V4L2_SUBDEV_FL_HAS_DEVNODE |	// 创建设备节点（/dev/v4l-subdevX）
		     V4L2_SUBDEV_FL_HAS_EVENTS;			// 支持事件通知机制
#endif
#if defined(CONFIG_MEDIA_CONTROLLER)
	ov8858->pad.flags = MEDIA_PAD_FL_SOURCE;	// 标记为源设备
